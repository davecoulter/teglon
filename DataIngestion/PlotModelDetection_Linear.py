...
for i in range(16):
    sub_dir = i+1
    results_table = Table.read("../Events/S190814bv/ModelDetection/Detection_Results_Linear_%s.prob" % sub_dir, format='ascii.ecsv')
...
model_tuples = []
# for i, (M, prob_arr) in enumerate(M_dM_probs.items()):
#     for j in range(len(model_X_input)):
#         model_tuples.append((M, model_X_input[j], prob_arr[j]))
...
# results_table = Table.read("../Events/S190814bv/ModelDetection/Detection_Results_Linear.prob", format='ascii.ecsv')
#
# M = np.asarray(results_table['M'])
# dM = np.asarray(results_table['dM'])
...
# for m in model_tuples:
#     # print(m)
#     clr = plt.cm.viridis(norm(m[2]))
#     ax.plot(m[1], m[0], 's', color=clr, markeredgecolor=clr, markersize=5) #markeredgecolor="black" , alpha=0.25
#     # ax.annotate("%0.2fc" % value[1], xy=(key[0], key[1]), xytext=(key[0], key[1]))
#     # ax.plot(m[0], m[1], '.', markersize=20)  # , alpha=0.25

ax.invert_yaxis()

manual_locations = [(10.0, -17.0), (8.0, -20.0), (5.0, -21.0), (3.0, -21), (0.0, -22.0)]
ax.contourf(xx, yy, z_new, levels=np.linspace(0.0, max_prob, 200), cmap=plt.cm.viridis) #[0.0, 0.10, 0.30, 0.5, 0.7, 0.9, max_prob]

CS = ax.contour(xx, yy, z_new, levels=[0.0, 0.10, 0.30, 0.5, 0.7, 0.9, max_prob], colors="None", linewidth=1.0)
