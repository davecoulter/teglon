...
# 			# parent_dict[int(row[1])] = int(row[0])

# 		for pi, pe in pixel_dict.items():

# 			# if pe.parent_pixel.index not in parent_dict:
# 			# 	print("NSIDE: %s" % nside) 
# 			# 	print("Child index %s, Parent index %s, not in parent_dict" % (pe.index, pe.parent_pixel.index))
# 			# 	raise("Exiting!")
			
# 			# parent_id = parent_dict[pe.parent_pixel.index]
...
# sky_pixel_query = "SELECT id from SkyPixel WHERE NSIDE = %s AND Pixel_Index = %s;"
sky_pixel_query = "SELECT id, Pixel_Index from SkyPixel WHERE NSIDE = %s;"
...
# child_galaxy_association_query1 = '''
# 	SELECT
# 		gd2.id 
# 	FROM GalaxyDistance2 gd2 
# 	JOIN SkyPixel_GalaxyDistance2 sp_gd2 ON sp_gd2.GalaxyDistance2_id = gd2.id 
# 	JOIN SkyPixel parent on parent.id = sp_gd2.SkyPixel_id 
# 	WHERE 
# 		gd2.z_dist IS NOT NULL and gd2.B IS NOT NULL AND 
# 		ST_CONTAINS((SELECT child.Poly FROM SkyPixel child WHERE child.Pixel_Index = %s AND child.NSIDE = %s), gd2.Coord) AND 
# 		parent.Pixel_Index in %s AND parent.NSIDE = %s
# '''



# Query free-text needs to have spaces *after* each line, as well as not terminate in semi-colon because MultiQueryDB will do that for us
# child_galaxy_association_query2 = '''
# 	SELECT 
# 		child.id as SkyPixel_id, gd2.id as GalaxyDistance2_id 
# 	FROM GalaxyDistance2 gd2 
# 	INNER JOIN SkyPixel_GalaxyDistance2 sp_gd2 ON sp_gd2.GalaxyDistance2_id = gd2.id 
# 	INNER JOIN SkyPixel parent on parent.id = sp_gd2.SkyPixel_id 
# 	INNER JOIN SkyPixel child on child.Parent_Pixel_id = parent.id 
# 	WHERE 
# 		gd2.z_dist IS NOT NULL and gd2.B IS NOT NULL AND 
# 		ST_CONTAINS(child.Poly, gd2.Coord) AND 
# 		parent.id in %s AND child.id = %s
# '''
...
# n128_pixel_select = '''
# 	SELECT id, Pixel_Index
# 	FROM SkyPixel sp 
# 	WHERE NSIDE = 128
# '''
# n128_pixel_result = QueryDB(n128_pixel_select)[0]
...
for i, (nside, pixel_dict) in enumerate(sky_pixels.items()):
	...
	if i == 0: # NSIDE = 2
		...
		for j, (pi, sp) in enumerate(pixel_dict.items()):
			...
			# sp_id = QueryDB(sky_pixel_query % (sp.nside, sp.index))[0][0]
			q = top_level_galaxy_association_query % (sp.id, sp.query_polygon_string)
			# print(q)

			batch_queries.append(q)
			# contained_galaxies_result = QueryDB(q)

			# for g in contained_galaxies_result:
				# phi = np.radians(float(g[1]))
				# theta = 0.5*np.pi - np.radians(float(g[2]))
				# n128_pixel_index = hp.ang2pix(nside32, theta, phi)
				# n128_id = next((p[0] for p in n128_pixel_result if p[1] == n128_pixel_index), None)

				# if n128_id is None:
				# 	raise("Orphaned galaxy!")

				# galaxy_association_data.append((sp_id, g[0])) #, n128_id
				# galaxy_association_data.append((sp.id, g[0])) #, n128_id

			# t2 = time.time()
			# print("SkyPixel-Galaxy NSIDE %s Query %s/%s - execution tile %s" % (nside, j+1, len(pixel_dict), (t2 - t1)))
		...
		for j, (pi, sp) in enumerate(pixel_dict.items()):
			...
			# pixel_parent_neighbor_indices = list(hp.get_all_neighbours(sp.parent_pixel.nside, sp.parent_pixel.index)) + [sp.parent_pixel.index]
			# print("sp index: %s" % sp.index)
			# print(pixel_parent_neighbor_indices)
			...
			# pixel_parent_string2 = "("
			# for p in pixel_parent_ids:
			# 	pixel_parent_string2 += "%s," % p
			# pixel_parent_string2 = pixel_parent_string2[:-1] + ")"


			# batch_queries2.append(child_galaxy_association_query2 % (pixel_parent_string2, sp.id))
			# print("\n")

			# contained_galaxies_result = QueryDB(child_galaxy_association_query % (sp.index, sp.nside, pixel_parent_string, sp.parent_pixel.nside))
			# if nside >= nside32:
			# 	time.sleep(0.08)
			...
			# 	# phi = np.radians(float(g[1]))
			# 	# theta = 0.5*np.pi - np.radians(float(g[2]))
			# 	# n128_pixel_index = hp.ang2pix(nside128, theta, phi)
			# 	# n128_id = next((p[0] for p in n128_pixel_result if p[1] == n128_pixel_index), None)
	...
	# t1 = time.time()
	# galaxy_association_data = MultiQueryDB(batch_queries)
	# t2 = time.time()

	# print("\n********* start DEBUG ***********")
	# print("Batch Query execution time: %s. Returned records: %s" % ((t2 - t1), len(galaxy_association_data)))
	# print("********* end DEBUG ***********\n")
	...
	print("\n\nNow let's INSERT these motherfuckers... (%s)" % len(galaxy_association_data))
	


	# for xyz in range(len(results1)):
	# 	if len(results1[xyz]) != len(results2[xyz]):
	# 		print("Mismatch!")
	# 		print(batch_queries1[xyz])
	# 		print(batch_queries2[xyz])
