...
class Teglon:
	...
	def main(self):
		...
		for p in map_pixels:
			...
			distance_probs = 1.0/np.sqrt(2.0*np.pi*dist_sigma**2)*np.exp(-1.0*(distance_arr-mean_dist)**2/(2*dist_sigma**2))
		...
		for ot in ot_result:
			t = Tile(float(ot[3]), float(ot[4]), swope.deg_width, swope.deg_height, healpix_map_nside, tile_id=int(ot[0]))
			t.field_name = ot[2]
			t.mjd = float(ot[8])
		...
		for ot in ot_result:
			t = Tile(float(ot[3]), float(ot[4]), nickel.deg_width, nickel.deg_height, healpix_map_nside, tile_id=int(ot[0]))
			t.field_name = ot[2]
			t.mjd = float(ot[8])
			t.mag_lim = float(ot[10])
			t.band_id = int(ot[7])
		...
		for ot in ot_result:
			t = Tile(float(ot[3]), float(ot[4]), thacher.deg_width, thacher.deg_height, healpix_map_nside, tile_id=int(ot[0]))
			t.field_name = ot[2]
			t.mjd = float(ot[8])
			t.mag_lim = float(ot[10])
			t.band_id = int(ot[7])
			...
			for band in pix_synopsis.measured_bands:
				...
				for model_param_tuple, model_dict in models.items():
					...
					for i, (mjd, delta_mjd) in enumerate(pixel_delta_mjd.items()):
						...
						pix_abs_mag = np.interp(delta_mjd, pix_synopsis.model_observer_time_arr_new[model_param_tuple], model_abs_mags)
