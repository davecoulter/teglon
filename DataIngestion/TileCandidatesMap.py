...
class Teglon:
	...
	def main(self):
		...
		candidate_file_path = "%s/ObservedTiles/%s" % (formatted_healpix_dir, self.options.candidate_file)

		target_file_name = candidate_file_path.split('/')[-1].split(".")[0]
		print("Target File name: %s" % target_file_name)


		resolved_candidate_file_path = "%s/ObservedTiles/%s" % (formatted_healpix_dir, target_file_name + "_resolved.txt")
