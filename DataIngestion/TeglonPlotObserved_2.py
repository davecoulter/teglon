...
class Teglon:
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        parser.add_option('--candidate_file', default="{FILENAME}", type="str",
                          help='Optional filename of candidates to plot.')
    ...
    def main(self):
        ...
        map_pix = [Pixel_Element(int(mp[2]), healpix_map_nside, float(mp[3]), pixel_id=int(mp[0])) for mp in
                   map_pix_result]
        ...
        for i, t in enumerate(observed_tiles["SINISTRO"]):
            t.plot(m, ax, edgecolor=clrs["SINISTRO"], facecolor=clrs["SINISTRO"], linewidth=0.25, alpha=tile_opacity)
        ...
        for i, t in enumerate(observed_tiles["KAIT"]):
            t.plot(m, ax, edgecolor=clrs["KAIT"], facecolor=clrs["KAIT"], linewidth=0.25, alpha=1.0, zorder=9999)
            t.plot(m, ax, edgecolor='k', facecolor="None", linewidth=0.25, alpha=1.0, zorder=9999)
        ...
        fig.savefig('GW190814_v2.png', bbox_inches='tight')  # ,dpi=840
        plt.close('all')
        print("... Done.")
