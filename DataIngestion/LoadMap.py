...
class Teglon:
	...
	def main(self):
		...
		if not build_map:
			...
			map_nside = hp.npix2nside(len(prob))
			...
			# new_prob = hp.ud_grade(prob, 256)
			# new_distmu = hp.ud_grade(distmu, 256)
			# new_distsigma = hp.ud_grade(distsigma, 256)
			# new_distnorm = hp.ud_grade(distnorm, 256)

			# print("New length:")
			# prob = new_prob
			# distmu = new_distmu
			# distsigma = new_distsigma
			# distnorm = new_distnorm
			# print(len(prob))

			
			header = dict(header_gen)
			npix = len(prob)
			print("\tNumber of pix in '%s': %s" % (self.options.healpix_file, len(prob)))

			sky_area = 4 * 180**2 / np.pi
			area_per_px = sky_area / npix
			print("\tSky Area per pix in '%s': %s [sq deg]" % (self.options.healpix_file, area_per_px))

			map_nside = hp.npix2nside(npix)
			print("\tResolution (nside) of '%s': %s\n" % (self.options.healpix_file, map_nside))

			# Initialize with 0.0 prob to galaxies 
			healpix_map_insert = "INSERT INTO HealpixMap (GWID, URL, Filename, NSIDE, t_0, SubmissionTime, NetProbToGalaxies) VALUES (%s,%s,%s,%s,%s,%s,%s);"
			healpix_map_data = [(self.options.gw_id, gw_url, self.options.healpix_file, map_nside, t_0, UTC_submission_time.strftime('%Y-%m-%d %H:%M:%S'), 0.0)]
		...
		if not build_pixels:
			...
			for i, n128_i in enumerate(N128_indices):
				...
				healpix_pixel_data.append((healpix_map_id, i, prob[i], distmu[i], distsigma[i], distnorm[i], mean[i], stddev[i], norm[i], N128_pixel_id))
		...
		if not build_completeness_func:
			...
			pixel_completeness_upload_csv = "%s/%s_pixel_completeness_upload.csv" % (formatted_healpix_dir, self.options.gw_id)
			...
			n128_pixel_id_list = [pixel_id for pixel_index, pixel_id in N128_dict.items()] # pixel_index, pixel_id are both INT
			...
			while j < k:
				...
				healpix_pixel_completeness_result = query_db([healpix_pixel_completeness_select % (healpix_map_id, batch_id_string)])[0]
		...
		if not build_galaxy_weights:
			...
			for g in galaxy_attributes:
				...
				galaxy_attribute_data.append((g[0], g[1], g[2], g[3], norm_4d_weight, norm_4d_weight*net_prob_to_galaxies))
			...
			# Create CSV, upload, and clean up CSV
			galaxy_attributes_upload_csv = "%s/%s_galaxy_attributes_upload.csv" % (formatted_healpix_dir, self.options.gw_id)
			try:
				t1 = time.time()
